...
export default async function LanguageHubPage({ params }: PageProps<"/learn/[language]">) {
  ...
  const curriculumPreview = curriculum.slice(0, 6);
  ...
  return (
    <>
      ...
      <section className="section"><div className="container"><SectionHeading eyebrow={`Curriculum · ${curriculum.length} lessons`} title="A real path, not a pile of exercises." intro={`Move through ${levels.join(", ")} with lessons rooted in home, work, people, travel, interests, and culture.`} /><div className="resource-grid resource-grid--preview">{curriculumPreview.map((lesson) => <Link className="resource-card" key={lesson.id} href={`/learn/${lesson.language}/${lesson.slug}`}><p className="eyebrow">{lesson.level} · {lesson.primaryEnvironment}</p><h3>{lesson.title}</h3><p>{lesson.learningObjective}</p></Link>)}</div></div></section>
