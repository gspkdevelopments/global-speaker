...
export function LanguageCard({language}:{language:LanguageProfile}){const {locale}=useInterfaceLocale();const c=copy[locale];return <article className={`language-card language-card--${language.accent}`}><div className="language-card__top"><span>{language.code}</span><i aria-hidden="true"/></div><h3>{language.nativeName}</h3><p>{language.eyebrow}</p><div className="language-card__themes">{language.themes.slice(0,4).map((theme)=><span key={theme}>{theme}</span>)}</div><Link href={`/learn/${language.key}`}>{c.explore} {language.nativeName} <span aria-hidden="true">↗</span></Link></article>}
